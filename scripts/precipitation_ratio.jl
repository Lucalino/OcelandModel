...
elseif constraint == "Ptot"
    ...
    ax.plot(s[ll:ul],(1-α) * Po[ll:ul],label="ocean precipitation  \$(1-α) P_o = (1-α) [E_o + α E_l(s) (1 - 1/Φ(s))]\$",linewidth=lw)
    ax.plot(s[ll:ul],α * Pl[ll:ul];label="land precipitation  \$α P_l = α E_l(s)/Φ(s)\$",lw)
    ax.plot(s[ll:ul],α * El[ll:ul]; label="land evapotranspiration  \$α E_l(s)\$",lw)
    ax.plot(s[ll:ul],(1-α) * Eo[ll:ul]; label="ocean evaporation  \$(1-α) E_o\$",lw)
    ax.plot(s[ll:ul],α * R[ll:ul]; label="runoff  \$α R = α [P_l - E_l(s)]\$",lw)
    ax.plot(s[ll:ul],(1-α) * Aol[ll:ul]; label="advection  \$(1-α) A = (1-α) (E_o-P_o)\$",lw=3.5,linestyle="dotted", color="deeppink")
    ax.plot(s[ll:ul], α * El[ll:ul] + (1-α) * Eo[ll:ul]; label="\$E_{tot} = α E_l + (1-α) E_o\$", lw, color="darkgreen")
    ax.plot(s[ll:ul], α * Pl[ll:ul] + (1-α) * Po[ll:ul]; label="\$P_{tot} = αP_l + (1-α) P_o\$", lw=3.5, linestyle="dotted", color="deepskyblue")
    axhline(0.0; linestyle="dotted", color= "black")
    ...
    #savefig(plotsdir("Closed Model/Fluxes/Ptot constrained/Water_fluxes_panel_report_v2.png"))
